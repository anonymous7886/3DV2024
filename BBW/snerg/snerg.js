...
function initFromParameters() {
  ...
  gRenderer2.autoClear = false;
  gRenderer2.setSize(canvas.width / 2, canvas.height / 2);
}
...
function loadOnFirstFrame() {
  ...
  allTexturesPromise.then(texture => {
    ...
    // for alpha
    textureAlpha = new THREE.Texture(canvas);

    let material = new THREE.ShaderMaterial({
      uniforms: {
        texture: { type: 't', value: textureAlpha },
      ...
      fragmentShader: `
        ...
        void main() {
          vec4 color = texture2D(texture, gl_FragCoord.xy / resolution);
          gl_FragColor = vec4(color.a, color.a, color.a, 1.0);
        }
      `
    });

    let fullScreenPlane2 = new THREE.PlaneBufferGeometry(canvas2.width, canvas2.height);
    let fullScreenPlaneMesh2 = new THREE.Mesh(fullScreenPlane2, material);
    fullScreenPlaneMesh2.position.z = -100;
    fullScreenPlaneMesh2.frustumCulled = false;
    gRayMarchScene2.add(fullScreenPlaneMesh2);
    gRayMarchScene2.autoUpdate = false;
...
function render(t) {
  ...
  gRenderer.clear();
  gRenderer2.setRenderTarget(null);
  gRenderer2.clear();
